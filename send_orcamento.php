...
function log_debug($message) {
    $log_file = 'debug_orcamento.log';
    $timestamp = date('Y-m-d H:i:s');
    file_put_contents($log_file, "[$timestamp] $message\n", FILE_APPEND | LOCK_EX);
}

log_debug("Iniciando processamento do formulário");
...
log_debug("Email destino: $to_email");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    log_debug("Método POST detectado");
    log_debug("Dados recebidos: " . json_encode($_POST));
    ...
    log_debug("Dados processados - Nome: $nome, Email: $email, Tipo: $tipo");
    ...
    log_debug("Preparando envio de email para: $to_email");
    log_debug("Headers: " . $headers);
    log_debug("Tamanho do corpo do email: " . strlen($email_body) . " caracteres");
    
    // Verificar se a função mail() está disponível
    if (!function_exists('mail')) {
        log_debug("ERRO: Função mail() não está disponível");
        echo json_encode([
            'success' => false,
            'message' => 'Sistema de email não configurado. Contate o administrador.',
            'debug' => 'Função mail() não disponível'
        ]);
        exit;
    }
    
    $email_enviado = mail($to_email, $subject, $email_body, $headers);
    
    log_debug("Resultado do envio: " . ($email_enviado ? 'SUCESSO' : 'FALHA'));
    ...
    if ($email_enviado) {
        ...
        log_debug("Log salvo com sucesso");
        ...
        log_debug("ERRO: Falha no envio do email");
        $error_message = error_get_last();
        log_debug("Último erro: " . ($error_message ? $error_message['message'] : 'Nenhum erro registrado'));
        
        echo json_encode([
            'success' => false,
            'message' => 'Erro ao enviar a solicitação. Por favor, tente novamente mais tarde.',
            'debug' => 'Falha na função mail() - verifique configuração do servidor'
        ]);
    }
